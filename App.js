...
export default function App() {
  ...
  const identificarPokemon = async () => {
    ...
    try {
      ...
      if (debugMode) {
        if (Platform.OS === 'web') {
           alert(`⚙️ TRAZA DE MOTORES\n\nRecorrido:\n${logIntentos.join('\n')}\n\nResultado Final:\n[${cleanName}]`);
        } else {
           Alert.alert("⚙️ TRAZA DE MOTORES", `Recorrido:\n${logIntentos.join('\n')}\n\nResultado Final:\n[${cleanName}]`);
        }
      }
      ...
      if (!res.ok) {
        alert(`La IA cree que es un "${rawName.trim()}", pero no logramos encontrarlo en la base de datos oficial. Prueba desde otro ángulo.`);
        setLoading(false);
        return;
      }
      ...
      if (errorMsg.includes('ERROR_CUOTA_O_MODELOS')) {
        Speech.speak("Alerta. Todos los modelos fallaron.", { language: 'es-ES', rate: 0.9 });
        if (debugMode) {
          Platform.OS === 'web' ? alert("🔥 FALLO GENERAL 🔥\n" + errorMsg) : Alert.alert("🔥 FALLO GENERAL 🔥", errorMsg);
        } else {
          alert("Todos los modelos han fallado (sin cuota o incompatibles).");
        }
      } else if (debugMode) {
        Platform.OS === 'web' ? alert("⚙️ TRAZA ERROR GRAVE\n" + errorMsg) : Alert.alert("⚙️ TRAZA ERROR GRAVE", errorMsg);
      } else {
        alert("Error en la conexión. Revisa tus datos móviles.");
      }
    } finally {
      setLoading(false);
    }
